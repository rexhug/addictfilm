...
function posterTile(m, { onClick, badge } = {}) {
  ...
  card.innerHTML = `
    <div class="art">
      ...
      ${m.poster_url ? `<img loading="lazy" decoding="async" src="${posterSrc(m.poster_url)}" alt="" ${_IMG_ERR}>` : ""}
...
async function showHome() {
  ...
  screen.innerHTML = `
    ...
    <section class="rise d5" id="sec-coll" hidden><div class="head"><h2>${esc(t("collections_title"))}</h2></div><div class="rail" id="rail-coll"></div></section>
...
async function loadCollectionsRail() {
  const rail = document.getElementById("rail-coll");
  const section = rail && rail.closest("section");
  try {
    ...
    rail.replaceChildren(...items.map(collectionCard));
  } catch (e) {
    ...
    <div class="detail-v2">
      ...
      <div class="d-body">
        ...
        ${cast.length ? `<div class="d-cast"><div class="d-cast-h"><h2>${esc(t("cast_title"))}</h2></div>
          <div class="d-cast-rail">${cast.map(a => `<div class="d-cast-item"><div class="d-avatar"><span class="fb">${esc(initials(a.name))}</span>${a.photo_url ? `<img loading="lazy" decoding="async" src="${posterSrc(a.photo_url)}" alt="" ${_IMG_ERR}>` : ""}</div><div class="n">${esc(a.name)}</div></div>`).join("")}</div></div>` : ""}
      </div>
    </div>`;
