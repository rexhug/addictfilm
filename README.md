...
- [ ] **Фаза B — поиск + кэш-каталог**: cache-first (фильм из API один раз),
      rate-limit под лимит kinopoisk (~200/сутки free).
- [ ] **Фаза C — discovery**: публичный каталог (популярное / топ спильноты /
      по жанрам), страница фильма с community-скором.
- [ ] **Фаза D — персональная статистика** с графиками.
- [ ] Ops: постоянный домен + Cloudflare Tunnel; при росте — платный тариф
      kinopoisk / SQLite→Postgres.
