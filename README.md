...
- [~] Деплой в облако: `Dockerfile` + `fly.toml` готовы и проверены (docker
      build/run локально), SQLite на постоянном томе, `DB_PATH` из env. Осталось
      выполнить шаги из **docs/DEPLOY.md** (нужен аккаунт хостинга + вход).
- [ ] Ops при росте: платный тариф kinopoisk / SQLite→Postgres.
