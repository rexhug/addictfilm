...
# Канонический состав фильма (cast_json): порядок титров, роли, устойчивые
# личности. Выключено → API отдаёт прежние actors/actors_photos, интерфейс
# работает как раньше. Колонка при этом наполняется, чтобы включение не
# требовало обхода каталога.
