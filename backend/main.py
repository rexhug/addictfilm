...
class VersionedStaticFiles(StaticFiles):
    ...
    Поэтому весь JS и CSS теперь revalidate: браузер спрашивает сервер и
    получает 304, если ничего не менялось. Это один условный запрос на запуск —
    цена, несопоставимая с зависшей версией интерфейса. Кэш картинок не
    затронут: у них URL меняется вместе с содержимым.

    Правило — по РАСШИРЕНИЮ, а не по списку имён. Список из двух файлов был
    верен ровно до появления frontend/js/*.js: новые модули не совпали ни с
    одним именем и поехали в Telegram вообще без Cache-Control, то есть на
    эвристическом кэше браузера. Любой следующий модуль повторил бы это молча.
    """

    _REVALIDATED_SUFFIXES = (".js", ".css")

    async def get_response(self, path, scope):
        response = await super().get_response(path, scope)
        if response.status_code == 200 and path.endswith(self._REVALIDATED_SUFFIXES):
